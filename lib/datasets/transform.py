...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        if 'saliency' in sample.keys():
            saliency = sample['saliency']
            saliency = cv2.resize(saliency, dsize=(new_w, new_h), interpolation=self.seg_interpolation)
            if self.fix:
                saliency = cv2.copyMakeBorder(saliency, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0])
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        saliency = saliency[top: top + new_h,
               left: left + new_w]
...
class RandomFlip(object):
    ...
    def __call__(self, sample):
        ...
        if np.random.rand() < self.flip_t:
            ...
            saliency_flip = np.flip(saliency, axis=1)
            sample['saliency'] = saliency_flip
...
class RandomRotation(object):
    ...
    def __call__(self, sample):
        ...
        new_saliency = cv2.warpAffine(saliency, m, (col,row), flags=self.seg_interpolation, borderValue=0)
...
class RandomScale(object):
    ...
    def __call__(self, sample):
        ...
        sal = cv2.resize(saliency, None, fx=rand_scale, fy=rand_scale, interpolation=self.seg_interpolation)
...
class Multiscale(object):
    ...
    def __call__(self, sample):
        ...
        for rate in self.rate_list:
            ...
            rescaled_saliency = cv2.resize(saliency, None, fx=rate, fy=rate, interpolation=cv2.INTER_NEAREST)
            sample['image_%f'%rate] = rescaled_image
            sample['saliency_%f'%rate] = rescaled_saliency
    ...
    def __call__(self, sample):
        ...
        for key in key_list:
            if 'image' in key:
                ...
                saliency = sample[key]
                sample[key] = torch.from_numpy(saliency.astype(np.float32))
